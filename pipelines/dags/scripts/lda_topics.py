...
dfTopcsKeywords = pd.DataFrame(extracted_topic_keywords, columns=('topic_Id',  'keyword_prob_scores', 'keyword'))
...
#make additional table topic keywords


writing_csv_start_time  = time.time()
dfTopcs = pd.DataFrame(extracted_topics, columns=('Id', 'name',))
dfTopcs.to_csv(outtopics, sep='\t', index = False)
sys.stderr.write("--- Writing %s : %s seconds, %i rows --- \n" % (outtopics,  time.time() - writing_csv_start_time, dfTopcs.shape[0]))


writing_csv_start_time  = time.time()
dfMemesTopics = pd.DataFrame(memeTopics, columns=('meme_Id', 'topic_Id', 'topic_prob_score'))
