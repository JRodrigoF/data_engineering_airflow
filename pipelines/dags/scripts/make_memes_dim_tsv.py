...
columns = ['kym_id', 'name', 'added', 'origin', 'status', 'year',
           'last_update', 'description', 'image_url', 'url', 'parent_kym_id', 'tags']
for col in columns:
    memes_dim[col] = []
...
for d in memes_dict:
    ...
    if not meme['details_year']:
        meme['details_year'] = 0
    memes_dim['year'].append(meme['details_year'])

    if not meme['last_update_source']:
        meme['last_update_source'] = 0
    ...
    memes_dim['tags'].append(json.dumps(
        sorted(meme['_tags']), ensure_ascii=False))
