...
"""
    ...
    Writes result in tsv file with columns:
        meme1_Id, meme2_Id, common_tags, tags_similarity, desc_similarity
...
result_table = {'meme1_Id':[], 'meme2_Id':[], 'common_tags':[], 'tags_similarity':[], 'desc_similarity':[]}
...
for i,j,v in zip(cx.row, cx.col, cx.data):
    ...
    #save only half of relations, as they are symmetric
    if df['Id'][i] > df['Id'][j]: continue
    result_table['meme1_Id'].append(df['Id'][i])
    result_table['meme2_Id'].append(df['Id'][j])
